...
class Genmax:
    ...
    def doGenmax(self,item_a, prefix_eq):
        # First Check
        ...
        for pattern_b in prefix_eq:
            if prefix_eq.index(pattern_b) > prefix_eq.index(item_a):
                pattern_ab = item_a.union(pattern_b)
                if len(pattern_ab.tids) >= self.minsupp:
                     prefix_eq_a.append(pattern_ab)
        if len(prefix_eq_a) > 0:
            for items in prefix_eq_a:
                self.doGenmax(items,prefix_eq_a)
        else:
            ########## Second Check
            found = False
            for pattern_z in self.maximal:
                if (item_a.itemset).issubset(pattern_z.itemset):
                    found = True
            if found == False :
                self.maximal.append(item_a)
...
def main():
    ...
    for items in initial_prefix_eq:
        my_genmax.doGenmax(items,initial_prefix_eq)
